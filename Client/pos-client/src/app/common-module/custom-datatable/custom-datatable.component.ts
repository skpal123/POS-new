...
export class CustomDatatableComponent implements OnChanges,OnDestroy,OnInit {
  ...
  btnClicked(columnName,data){
   this.customButtonClick.emit({columnName:columnName,rowData:data})
  }
  checkboxClick(columnName,data,$event){
    this.customCheckboxClick.emit({columnName:columnName,rowData:data,isChecked:$event.target.checked})
  }
  saveNewOrder(){
